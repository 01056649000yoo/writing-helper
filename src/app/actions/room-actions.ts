...
function parseQuestionGeneratorSetupPayload(formData: FormData): QuestionGeneratorSetupInput {
  ...
  // 값이 없으면 목표와 같게 둔다 — 선생님이 정한 개수를 다 채우게 하는 것이 기본이다.
  const minSelections = raw.minSelections !== undefined
    ? Number(raw.minSelections)
    : Number(formData.get("min_selections"));

  return {
    mode: parseQuestionGeneratorMode(raw.mode),
    selectedCardSetIds,
    customQuestions,
    minSelections: Number.isFinite(minSelections) ? minSelections : Number.NaN,
...
export async function generateQuestionsWithAI(
  ...
  // ⚠️ 완성된 질문을 주면 학생이 그대로 베껴 낸다. 질문 카드(비계)와 같은 결로
  //    **질문을 만들 관점**을 주는 문장을 만들게 한다 — 끝맺음은 "~ 질문을 만들어 보세요."
  const prompt = `초등학교 글쓰기 수업 전 "질문 만들기(생각 열기)" 활동에 쓸 **질문 카드**를 만듭니다.
...
질문 카드는 **완성된 질문이 아니라, 학생이 스스로 질문을 만들도록 관점을 열어 주는 안내 문장**입니다.
학생은 이 카드를 읽고 자기 질문을 직접 씁니다.

반드시 지킬 것:
1. 각 문장은 **"~ 질문을 만들어 보세요."** 로 끝납니다. 물음표(?)로 끝나는 완성된 질문은 절대 쓰지 마세요.
2. 먼저 **어디를 볼지**(장면·인물·물건·시간·내 경험 같은 살펴볼 지점)를 짚어 주고, 그 다음 **무엇을 물을지**의 방향을 알려 줍니다.
3. ${count}개가 서로 다른 관점이어야 합니다(상상·마음·감각·이유·연결·관점 등이 겹치지 않게).
4. 초등학생이 읽고 바로 이해할 수 있는 짧고 친근한 한 문장으로 씁니다.
5. 주제와 관계없는 일반론은 쓰지 않습니다. 위 주제의 내용에 붙여서 씁니다.

좋은 예:
- "이야기가 끝난 다음 날 주인공에게 무슨 일이 생길지 상상해 보고, 그것을 묻는 질문을 만들어 보세요."
- "가장 또렷이 떠오르는 장면을 하나 고르고, 그곳에서 어떤 소리가 들렸을지 묻는 질문을 만들어 보세요."
- "겉으로는 화났지만 속마음은 달랐을 것 같은 인물을 찾아, 그 진짜 마음을 묻는 질문을 만들어 보세요."

나쁜 예(이렇게 쓰지 마세요):
- "주인공은 왜 그런 선택을 했을까?"  ← 완성된 질문이라 학생이 그대로 베낍니다.
- "가장 기억에 남는 장면은 무엇인가요?"  ← 완성된 질문입니다.
...
JSON 형식으로만 응답:
{
  ...
  try {
    ...
    if (!Array.isArray(parsed.questions)) {
      return {
        questions: [
          `${topic}에서 가장 인상 깊었던 장면을 하나 고르고, 그 장면에 대해 묻는 질문을 만들어 보세요.`,
          `내가 ${topic} 속 주인공이 되었다고 생각하고, 그때 무엇을 하고 싶을지 묻는 질문을 만들어 보세요.`,
          `${topic}을(를) 처음 만났을 때 들었던 생각이나 기분을 떠올리고, 그것을 묻는 질문을 만들어 보세요.`,
          `${topic}에서 아직 잘 모르겠는 점을 하나 찾아, 친구들과 함께 이야기할 질문을 만들어 보세요.`,
          `${topic}을(를) 겪기 전과 뒤에 달라진 내 생각을 떠올리고, 그 변화를 묻는 질문을 만들어 보세요.`,
        ],
      };
    }
